	...
	$("#moveOn").on("click", function(){
		...
		if (p==1 || p==3 || p==5 || p==7 ||p==9){
			...
			$(".name").fadeOut(1000);
				...
				$("#showRole").empty();
				var assignedLeaderIndex = assignLeader(-1);
...
function look(assignedLeaderIndex) {
	...
	$("#look").append("<h2 id='h2 s'>The evil players will now find their teammate!</h2>");
...
function acceptDenyNominatedPlayers(nominated, assignedLeaderIndex){
    ...
    document.getElementById("main").innerHTML = "<h1>Vote by raise of hands to send the nominated players on mission!</h1>"
            + "<h2 id='players'></h2>"
            + "<h1 id='result'></h1>"
...
function votesFunction(nominated, assignedLeaderIndex){
	...
	*/
	...
	$("#vote").append("<h1 class='name' id='name'></h1>");
	 $("#vote").append("<h1 class='name' id='instructions'>The nominated players will now vote to either try to fail or succeed the mission. </h1>")
...
function outCome(voteList, assignedLeaderIndex){
	...
	for (var i = 0; i < voteList.length; i++){
		...
		$("#votes").hide().append('<li class="listInline">' + voteList[i] + '</li>').fadeIn(2000);
	}
